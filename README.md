...
scored 100,000 candidates 
wrote 100 rows to submission.csv
```

Well inside the 5-minute / 16GB CPU-only budget, with roughly 2x-3x time
headroom and 4x-6x memory headroom. There is no model loading and no
per-candidate I/O, so this scales linearly and predictably.
